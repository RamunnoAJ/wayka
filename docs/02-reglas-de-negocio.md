...
| Política de contraseña | Usuario | Mínimo 8 caracteres, con al menos una letra minúscula, una mayúscula y un dígito. Máximo 72 **bytes** — los caracteres acentuados y la ñ ocupan dos, así que se llega al tope con menos letras de las que dice el número. El máximo no es una decisión de producto sino el límite de bcrypt, que rechaza lo que lo supere: sin validarlo, una contraseña larga terminaba en un error del servidor en vez de en un rechazo que diga qué corregir. Rige tanto el alta como el cambio de contraseña. |
