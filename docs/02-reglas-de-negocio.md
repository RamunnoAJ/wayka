...
| Regla | Aplica a | Validación |
|---|---|---|
| Tutor y Veterinario, los dos canales | Usuario (tipo_usuario = tutor, veterinario) | Pueden autenticarse desde web y desde móvil. |
| Clínica_admin solo por web | Usuario (tipo_usuario = clínica_admin) | Todo intento de autenticación desde el canal móvil se rechaza, independientemente de que el email y la contraseña sean correctos. |

> **El tutor entró a la web después de la primera versión de esta regla.** Decía que no accedía bajo ninguna circunstancia, y en la práctica dejaba afuera al tutor que no tiene el teléfono a mano. Lo que sigue siendo exclusivo de la app son el push y la cámara, que dependen del aparato y no de un permiso: **ningún dato ni ninguna acción cambian con el canal**. El bloqueo que queda en pie es el del clínica_admin, que es de producto y no de dispositivo.
...
1. **Canal**: notificación push al móvil del tutor. La cuenta registra sus dispositivos al iniciar sesión **desde la app**; una cuenta sin dispositivo registrado no recibe avisos, y eso incluye al tutor que solo entró alguna vez por web (regla 2.3). El email queda descartado como segundo canal: no se agrega un medio nuevo para el mismo destinatario mientras el push cubra el caso.
