...
| Telemetría de producto | No queda afuera: los eventos de uso se encolan y suben con este mismo ciclo, pero no son estado replicado ni mutaciones del dominio. Su contrato completo está en Telemetría de Producto; acá solo se define cómo viajan (sección 5). |
...
**La Consulta atendida no se replica** (Modelo de Datos, 4.16): el tutor no la lee ni online, así que la invariante de más abajo la deja afuera sola. Lo que el tutor ve de una atención es el historial que se escribió, no el asiento de que ocurrió.
...
- **Los datos no clínicos de la mascota se editan con o sin señal**, y son una mutación aparte de la del peso. El peso tiene un tipo propio desde antes porque es lo único que el tutor toca a diario; nombre, especie, raza, fecha de nacimiento y sexo se corrigen una vez y se corrigen tarde —el nombre mal tipeado en el alta se descubre semanas después—, y esperar señal para eso no tiene ningún argumento a favor. Los dos motivos por los que algo queda afuera de la cola no aplican acá: no es una revocación en diferido y la mascota ya existe, así que no hay identificador que inventar. El número de chip **no** viaja en esta mutación: lo carga el veterinario (Reglas de Negocio, 3.2).

- **Compartir y revocar no pasan por la cola.** Son escrituras del tutor que quedan afuera a propósito: encolar una revocación es revocar en diferido, que es la peor versión del problema que la sección 8 describe. Sin señal, la aplicación lo dice y no ofrece la acción. Por el mismo motivo queda afuera el alta de una mascota, aunque el argumento ahí es otro: encolarla obligaría a inventar identificadores locales y a reconciliarlos después, un mecanismo entero para una pantalla que se usa dos veces en la vida.
- **Los antecedentes que carga el tutor sí pasan por la cola** (Reglas de Negocio, 4.23), y son la primera escritura *clínica* del tutor que entra acá. Es un alta, no una edición, y por eso vale la pena decir por qué no arrastra el problema que dejó afuera al alta de mascota: el antecedente cuelga de un `paciente_id` **que ya existe y ya está en la copia local**, así que no hay ningún identificador que inventar hacia arriba. El registro nuevo se muestra como pendiente usando el identificador de su propia mutación como clave local, y cuando la sincronización lo baja con su id real por la bitácora (sección 3), la fila pendiente se reemplaza. Nada más lo referencia mientras tanto: un antecedente recién cargado no tiene hijos.
  - Un alta **no lleva `version_base`**, y es la primera de la cola que no lo hace: no hay registro previo cuyo `updated_at` comparar. Lo que la puede rechazar no es una carrera contra otra escritura sino una regla de negocio —la droga ya activa, la mascota dada de baja, el acceso revocado— y esas las decide el servidor igual que online.
  - **El `entidad_id` de un alta es la mascota**, no el registro que se va a crear. Es un cambio de significado del campo respecto de las otras cuatro mutaciones, y se dice explícito porque leer "entidad_id" como "el id del registro que cambia" es lo natural.
  - Un alta rechazada **sale de la copia local** —no forma parte del historial— y su contenido completo queda en la cola, que es lo que la pantalla de rechazos muestra para poder corregirlo. Es el único caso en que descartar un rechazo tira el único lugar donde el dato estaba escrito, y por eso ahí se ofrece corregir antes que descartar.
- **La carga de antecedentes del onboarding, en cambio, exige conexión de punta a punta.** No es una excepción a lo anterior sino su consecuencia directa: ahí la mascota todavía no existe —el alta no pasa por la cola— y no hay `paciente_id` del que colgar nada. La pantalla lo dice antes de que el tutor escriba, no después. Desde la ficha, con la mascota ya creada, la misma pantalla funciona sin señal.
...
- **La telemetría viaja aparte y es descartable.** Los eventos de uso que el cliente genera sin conexión se encolan también en el dispositivo y suben con este mismo ciclo, pero por su propia ruta y con dos diferencias: su cola tiene techo y **descarta lo más viejo** cuando se llena —una mutación no se descarta nunca— y nunca demora ni bloquea la subida de mutaciones. El dato clínico va primero, siempre (Telemetría de Producto, 7).
...
La superficie de escritura del tutor es corta y sigue siéndolo, aunque haya crecido: a los datos no clínicos de la mascota y a la agenda se sumaron los **antecedentes** (Reglas de Negocio, 4.23), y lo que se agregó son altas, que es la clase de mutación que menos conflictos produce. Lo que sí cambió antes es que puede haber **más de una persona escribiendo** sobre la misma mascota:
...
| **Alta de un antecedente** (Evento clínico o Medicación con `cargado_por = tutor`) | Ninguno con otra escritura: nace un registro nuevo y no pisa nada | Se aplica. El único rechazo posible es de validación o de permiso |
| Alta de una **Medicación** declarada | El paciente ya tiene esa droga activa —la cargó la clínica, o el otro tutor— (Reglas de Negocio, 2.2) | Rechazo por regla de negocio. El motivo dice cuál es la activa que estorba y quién la cargó |
| Edición o baja de un antecedente propio | El veterinario lo corrigió o lo dio de baja mientras tanto (Reglas de Negocio, 3.2) | Concurrencia optimista; si ya no está, se informa y la mutación se descarta |
...
**Un antecedente rechazado no se pierde de vista.** Es la única mutación de la cola que representa algo que el tutor no puede volver a mirar en ningún lado: el peso está en la ficha y la reagenda en el calendario, pero "la vacuna antirrábica de 2023" solo existe en la cola hasta que entra. Por eso el rechazo muestra el contenido completo de lo que se quiso cargar y ofrece corregirlo ahí mismo, en vez de solo descartarlo — descartar es tirar el único lugar donde el dato estaba escrito.
