...
| Panel de métricas en la aplicación | En el MVP las métricas de **este documento** se consultan por SQL contra la base. Ninguna cuenta lee telemetría por la API (sección 6). Construir un panel de producto antes de saber qué números se miran todos los días es construirlo dos veces. No es el tablero del clínica_admin (Alcance de Plataformas, 3.2.1): ese lee conteos de entidades del dominio —Cita, Consulta atendida, Vínculo con clínica—, no `Evento de telemetría`, y por eso no reabre esta decisión ni la de la sección 6. |
